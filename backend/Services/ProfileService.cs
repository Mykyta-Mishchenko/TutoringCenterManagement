...
namespace backend.Services
{
    public class ProfileService : IProfileService
    {
        ...
        public async Task<byte[]> GetUserProfile(int userId)
        {
            var filePath =  await _userRepository.GetUserProfile(userId);
        ...
        public async Task<string> SetUserProfile(string refreshToken, IFormFile file)
        {
            var session = await _sessionRepository.GetSession(refreshToken);

            if (session != null)
            {
                var imgPath =  await _userRepository.GetUserProfile(session.UserId);
                ...
                return await SaveProfileImg(session.UserId, file);
            }
            return string.Empty;
        }
        ...
        private async Task<string> SaveProfileImg(int userId, IFormFile file)
        {
            ...
            await _userRepository.SetUserProfile(userId, filePath);
            return filePath;
        }
    }
}
